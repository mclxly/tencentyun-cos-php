...
class Api {
   ...
   /*
    ...
    /**
     * Upload entire folder, overwrite exist files.
     * @param $bucket bucket name.
     * @param $srcDir src folder.     
     * @param $bucketBaseDir
     * @return array|mixed.
     */
    public function uploadFolder($bucket, $srcDir, $bucketBaseDir)
    {
        $objects = new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator($srcDir), \RecursiveIteratorIterator::SELF_FIRST);
        foreach($objects as $name => $object){
            ...
            // 上传文件
            if (strpos($name, $bucketBaseDir) === false) {
                echo "Wrong file: $name!\n";
                continue;
            }
            $src = $name;
            $dst = substr($name, strlen($bucketBaseDir));

            echo "$src -> $dst...";

            $ret = $this->upload($bucket, $src, $dst, null, null, 0);            
            // var_dump($ret);
